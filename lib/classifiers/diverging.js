 ...
 */
const diverging = ({scheme = equal, center = 0, ...opts} = {}) => (
  grid,
  nBreaks
) => {
  ...
  const breaksBelow = dataBelow.length > 0
    ? classifier(
        {...grid, data: dataBelow, max: Math.min(grid.max, center)},
        breaksPerSide
      ).slice(0, breaksPerSide - 1)
    : range(breaksPerSide - 1).map(i => center)
  const breaksAbove = dataAbove.length > 0
    ? classifier(
        {...grid, data: dataAbove, min: Math.max(grid.min, center)},
        breaksPerSide
      )
